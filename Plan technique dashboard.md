...
**Variables d'environnement** (`.env.example`) : `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `DASHBOARD_PW_EQUIPE`, `DASHBOARD_PW_PATRON`, `SESSION_SECRET`, `FAL_KEY`, `FAL_IMAGE_MODEL`, `FAL_TEXT_MODEL`, `FAL_TEXT_LLM`, `STRIPE_SECRET_KEY` (remboursements, 2026-07-19 — même clé que le site), `RESEND_API_KEY` (e-mails expédition/remboursement, 2026-07-19 — même clé que le site ; absente = flux dégradé sans e-mails).
...
24. Étiquette produit imprimable (code-barres Code128 maison `lib/barcode.ts`, route `/etiquette/[id]`) · Remboursement de commande (refund Stripe automatique + e-mail client [2026-07-19] + statut + remise en stock) · Expédition (`marquerExpediee` : CAS payee→expediee + `numero_suivi` + e-mail client ; affichage **« À expédier » en orange** pour en_ligne+payee via `statutAffichage()` [constants], liste + détail, 2026-07-19) · Responsive tablette/téléphone (nav mobile tiroir + tableaux scroll) · Tickets vente/rachat imprimables (`/ticket/vente/[id]`, `/ticket/rachat/[id]`)
...
- **Base de données** : 9 tables (categories, sous_categories, produits, clients, commandes, ventes, rachats, transactions_or, **produit_image_jobs**), 25 migrations. Projet Supabase ref `nztqbfxsaduockzilrve` (région Frankfurt). Depuis le paiement Stripe (site, 2026-07-09) : une commande en ligne naît `en_attente` (stock déjà réservé) et ne passe `payee` que via le webhook Stripe — les KPI du Tableau de bord ne comptent que `payee/expediee/livree` (inchangé). La liste Ventes **masque les `annulee`** (bruit des paiements abandonnés — conservées en base, `listCommandes` filtre `.neq statut annulee`) ; les `en_attente` restent visibles. Le bouton Remboursement fait AUSSI le **refund Stripe automatique** depuis le 2026-07-19 (`stripe_session_id` → `refunds.create`, remboursement total dans la devise encaissée ; Stripe d'abord, DB ensuite ; env `STRIPE_SECRET_KEY` requise côté dashboard) — les ventes magasin restent DB seule, argent rendu à la main.
