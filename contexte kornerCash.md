...
- **25 migrations**. Dernières : `add_code_passeport_clients` (2026-07-04), `add_stripe_session_id_commandes` (2026-07-09, paiement Stripe), `add_taux_eur_commandes` (2026-07-19, taux EUR figé par commande). Détail complet : `database architecture.md`.
...
- **Paiement Stripe** : **en prod (mode test) depuis le 2026-07-19** — testé 8/8 en local (2026-07-09), env Vercel + endpoint webhook prod posés, remboursement automatique branché côté dashboard (commit `e87b9ae`), Klarna retiré. **Re-test prod validé le 19-07** (achat + 3 e-mails + refund Stripe vérifié `succeeded`). Reste : passage en clés live à la mise en service réelle.
- **E-mails transactionnels + taux EUR réel (2026-07-19)** : confirmation de commande (webhook), expédition (nouveau flux dashboard « Marquer expédiée » + n° de suivi, badge **« À expédier » orange** pour les commandes en ligne payées) et remboursement — envois jamais bloquants, mode test Resend, **les 3 testés et reçus en prod**. Taux CHF→EUR vivant (Frankfurter, repli 1.06) figé sur chaque commande EUR (`taux_eur`). Panier : resync `storage`/`pageshow` (`324cb41`) — le panier vidé après paiement ne ressuscite plus via bfcache/autre onglet.
