...
**État général (session 2026-07-09)** : Les deux apps sont **EN LIGNE** sur Vercel — dashboard https://kornercash-dashboard.vercel.app et site https://kornercash-site.vercel.app (repos git connectés = auto-deploy). La **feature IA photos & descriptions** (dashboard, fal.ai) est **déployée + validée en prod** (189 générations bijoux, 0 échec) ; l'**Atelier** a permis d'intégrer ~180 bijoux réels. Le **paiement Stripe Checkout est CODÉ + TESTÉ 8/8 par Anatole en local** (2026-07-09, commits locaux, mode test — pas encore déployé : reste env Vercel + webhook prod + push ; 2 décisions ouvertes : remboursement Stripe auto + Klarna). Restent aussi : DNS `kornercash.ch` (accès domaine pas encore obtenu), taux EUR réel, e-mails transactionnels (Resend, domaine à vérifier), photos du reste du stock, coordonnées légales (`entreprise.ts`) + validation fiduciaire des tickets.
...
- [ ] **Paiement Stripe — mise en prod** : code prêt et testé 8/8 en local (voir Phase 4). Séquence restante : poser `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` dans Vercel → créer l'endpoint webhook prod (`/api/stripe/webhook`, il fournit le `whsec_…`) → push (site `master` : commits locaux en attente) → basculer en clés live à la mise en service réelle (compte Stripe au nom du client). NB : l'expiration auto d'une session abandonnée (stock restitué après 35 min) ne fonctionne qu'avec le webhook — donc seulement en prod ou avec `stripe listen` en local.
- [ ] **DÉCISION — Remboursement Stripe automatique** : le bouton Remboursement du dashboard ne fait QUE la DB (statut + restock) — **il ne rend PAS l'argent au client**. Aujourd'hui il faut rembourser à la main dans le Dashboard Stripe (Payments → Refund). Reco Claude : brancher `stripe.refunds.create` dans `rembourserCommande` (la commande porte `stripe_session_id` ; nécessite dep `stripe` + `STRIPE_SECRET_KEY` côté dashboard, ~1 h). **En attente de décision d'Anatole.**
- [ ] **DÉCISION — Klarna** : apparu automatiquement sur la page de paiement (moyens dynamiques Stripe). = paiement différé/en plusieurs fois : le client étale, KornerCash est payé comptant, frais plus élevés (~4,99 % + 0.35 vs 2,9 % carte). Peut aider la conversion sur les articles chers (or, consoles). **À trancher avec le patron** — désactivable sans code : Dashboard Stripe → Settings → Payment methods.
