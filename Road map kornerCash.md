...
- [x] **Paiement Stripe — mise en prod mode test FAITE (2026-07-19)** : env Vercel posées (site + dashboard) + endpoint webhook test `we_1Tuq6v1ywJcOEms7Udyg8pj9` créé via API. **Reste à la mise en service réelle** : basculer en clés live (compte Stripe au nom du client) + créer l'endpoint webhook LIVE (nouveau `whsec_`) + mettre à jour l'URL du webhook quand le DNS `kornercash.ch` sera branché. NB : l'expiration auto d'une session abandonnée (stock restitué après 35 min) ne fonctionne qu'avec le webhook — désormais actif en prod.
